...
foreach my $infile( @infiles) {
    ...
    if( -f "done/$mp4_file" ){
      ...
      my $mp4_duration = qx(ffmpeg -i done/${mp4_file} 2>&1 | grep "Duration");
      $mp4_duration =~ s/^\s+(\S.+?\S)\s+$/$1/;

      print("mp4: [$mp4_duration]$/") if $DEBUG;
      ...
      if( $dv_duration{Duration} eq $mp4_duration{Duration}
        ...
        print("duration of raw data matches duration of mp4.  Next.$/") if $DEBUG;
        next;
    ...
    if( $? == 0 ){
      print("moving mp4 file to done/$/") if $DEBUG;
